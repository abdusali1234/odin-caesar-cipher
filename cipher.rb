...
def caesar_cipher(string, right_shift_val)
  string.chars.reduce('') do |message, char|
    ...
    rotation = if ascii_val.between?(ASCII_UPCASE_MIN,
                                     ASCII_UPCASE_MAX) || ascii_val.between?(ASCII_DOWNCASE_MIN, ASCII_DOWNCASE_MAX)
                 (((ascii_val - base) + right_shift_val) % 26) + base
               else
                 ascii_val
               end
